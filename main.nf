...
workflow {
    ...
    // -------------------------------------------------------------------------
    // UNIFIED DATABASE PATH RESOLUTION (SAME EXACT CLEAN LOGIC FOR ALL THREE)
    // -------------------------------------------------------------------------
    ...
    def finalInterproscanPath = (optionalParams.interproscan_DB && file(optionalParams.interproscan_DB).exists()) ?
        file(optionalParams.interproscan_DB) : file("${launchDir}/work/interproscan_data")
    ...
    if (params.mode == 'both' || params.mode == 'funannotate') {
        def safeFileFallback = { pathStr, dummyFile ->
            if (pathStr && pathStr.toString() != "" && !pathStr.toString().contains('NO_') && file(pathStr).exists()) {
                return file(pathStr)
            }
            return dummyFile
        }

        def nano_file = safeFileFallback(optionalParams.nanopore_mrna, dummy_files[11])
        def pb_file   = safeFileFallback(optionalParams.pacbio_isoseq, dummy_files[12])
    ...
    if (params.mode == 'both' || params.mode == 'braker') {
        ...
        def genome_unmasked_ch = Channel.of(file(mandatoryParams.genome_unmasked))
        def script_dir_ch      = Channel.of(file(params.script_dir))

        BRAKER(
            braker_input,
            genome_unmasked_ch,
            trna_gff_channel.map { _, g -> g },
            script_dir_ch
        )
    }
    ...
    def br_proteins = (params.mode in ['both', 'braker'])      ? BRAKER.out.proteins      : Channel.of([fullMeta, no_prot]).ifEmpty([fullMeta, no_prot])
    def br_gff3     = (params.mode in ['both', 'braker'])      ? BRAKER.out.gff3          : Channel.of([fullMeta, no_gff]).ifEmpty([fullMeta, no_gff])

    // =============== UNIFIED MERGE AND EVALUATION ===============
    if (params.mode == 'both') {
        def combined_input = FUNANNOTATE.out.busco_summary
            .combine(BRAKER.out.busco_summary, by: 0)
            .combine(FUNANNOTATE.out.proteins, by: 0)
            .combine(FUNANNOTATE.out.gff3, by: 0)
            .combine(BRAKER.out.proteins, by: 0)
            .combine(BRAKER.out.gff3, by: 0)
            .combine(trna_gff_channel, by: 0)
            .combine(Channel.of([fullMeta, file(mandatoryParams.genome_unmasked)]), by: 0)
            .map { meta, fa_busco, br_busco, fa_prot, fa_gff, br_prot, br_gff, trna_gff_file, genome ->
                [meta, fa_busco, br_busco, fa_prot, fa_gff, br_prot, br_gff, trna_gff_file, genome]
            }

        MERGE_ANNOTATIONS(combined_input, Channel.of(file(params.script_dir)))
    }
